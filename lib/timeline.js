...
// Boundary markers (turn/step start+end) render as thin dividers rather than
// full cards on the client - they're needed for orientation but shouldn't
// dominate a timeline that's mostly tool calls and messages.
const COMPACT_TYPES = new Set(['turn/start', 'turn/end', 'step/start', 'step/end']);

/**
 * Paginated timeline reduction. Without `beforeSeq`, returns the most recent
 * `limit` kept events (what a session detail page opens to). With
 * `beforeSeq`, returns the `limit` kept events immediately before that seq,
 * for a "load earlier" control - a 489-event session becomes a handful of
 * bounded pages instead of one huge scroll.
 */
function buildTimeline(events, { limit = 100, beforeSeq = null } = {}) {
  let kept = events.filter(e => KEPT_TYPES.has(e.type));
  if (beforeSeq != null) kept = kept.filter(e => (e.seq ?? -Infinity) < beforeSeq);
  const total = kept.length;
  const page = kept.slice(-limit);
  const hasMore = total > page.length;
  const oldestSeq = page.length ? page[0].seq : null;

  const timeline = page.map(ev => ({
    seq: ev.seq ?? null,
    time: ev.time ?? null,
    type: ev.type,
    compact: COMPACT_TYPES.has(ev.type),
    ...summarize(ev),
  }));
  return { timeline, hasMore, oldestSeq };
}
