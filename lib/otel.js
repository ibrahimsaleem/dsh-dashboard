...
//
// dsh-session-telemetry-otel's own README says FULL mode ships raw event
// content with no redaction of its own - see lib/redact.js, applied here at
// ingest time, default on (DASHBOARD_OTEL_REDACT=false to disable).

const express = require('express');
const { redactValue, isEnabled: redactionEnabled } = require('./redact');
...
function ingestLogRecord(record, sourceHost) {
  ...
  let data = decodeAnyValue(record.body);
  if (redactionEnabled()) data = redactValue(data);
  sess.eventsBySeq.set(seq, { type, seq, time, data });
}
