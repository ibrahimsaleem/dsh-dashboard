...
function buildDashboard(dshHome) {
  ...
  for (const sf of sessionFiles) {
    ...
    sessions.push({
      ...
      origin: sessionMeta?.origin || 'top-level',
      parentSession: sessionMeta?.parentSession || null,
...
function getSessionTimeline(dshHome, sessionId, limit) {
  ...
  for (const sf of fileSources) {
    ...
    if (id === sessionId || sf.sessionDir === sessionId) {
      return { sessionId: id, source: 'local', title: meta?.cwd, timeline: buildTimeline(events, { limit }) };
    }
  }
  const otelMatch = listOtelSessions().find(s => s.sessionId === sessionId);
  if (otelMatch) {
    return { sessionId, source: 'otel', title: otelMatch.cwd, timeline: buildTimeline(otelMatch.events, { limit }) };
  }
  return null;
}
